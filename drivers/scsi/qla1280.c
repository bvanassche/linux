	...
	__must_hold(cmd->device->host->host_lock)
{
	...
	/* Tell the compiler that the two locks mentioned below are the same. */
	__release(cmd->device->host->host_lock);
	__acquire(((struct scsi_qla_host *)(CMD_HOST(cmd)->hostdata))->host->host_lock);
	...
	__release(((struct scsi_qla_host *)(CMD_HOST(cmd)->hostdata))->host->host_lock);
