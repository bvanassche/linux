...
static void max77620_gpio_bus_lock(struct irq_data *data)
	__acquires(&((struct max77620_gpio *)gpiochip_get_data(irq_data_get_irq_chip_data(data)))->buslock)
{
	struct gpio_chip *chip = irq_data_get_irq_chip_data(data);
	struct max77620_gpio *gpio = gpiochip_get_data(chip);
...
static void max77620_gpio_bus_sync_unlock(struct irq_data *data)
	__releases(&((struct max77620_gpio *)gpiochip_get_data(irq_data_get_irq_chip_data(data)))->buslock)
{
	struct gpio_chip *chip = irq_data_get_irq_chip_data(data);
	struct max77620_gpio *gpio = gpiochip_get_data(chip);
